...
describe("CONTRACT_STATUS_LABELS", () => {
    it("todos os status têm label definida", () => {
        ...
        for (const s of statuses) {
            expect(CONTRACT_STATUS_LABELS[s])
                .to.be.a("string")
                .and.not.equal("");
        }
    });
    ...
    it("labels correspondem aos valores esperados", () => {
        expect(CONTRACT_STATUS_LABELS[ContractStatus.PENDING]).to.equal(
            "Pending",
        );
        expect(CONTRACT_STATUS_LABELS[ContractStatus.ACTIVE]).to.equal(
            "Active",
        );
        expect(CONTRACT_STATUS_LABELS[ContractStatus.RESCINDED]).to.equal(
            "Rescinded",
        );
        expect(CONTRACT_STATUS_LABELS[ContractStatus.EXPIRED]).to.equal(
            "Expired",
        );
    });
});
...
describe("RIGHTS_MINTER ABI", () => {
    ...
    it("contém executedAgreements", () =>
        expect(names).to.include("executedAgreements"));
...
describe("VAULT_FACTORY ABI", () => {
    ...
    it("contém getVaultsByClub", () =>
        expect(names).to.include("getVaultsByClub"));
});
...
describe("RIGHTS_VAULT_ABI", () => {
    ...
    it("contém depositCaution", () =>
        expect(names).to.include("depositCaution"));
    it("contém rescindByPlayer", () =>
        expect(names).to.include("rescindByPlayer"));
    it("contém rescindByClub", () => expect(names).to.include("rescindByClub"));
    it("contém expireContract", () =>
        expect(names).to.include("expireContract"));
