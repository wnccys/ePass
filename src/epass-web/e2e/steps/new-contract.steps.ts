...
Then("I should see the contract title field", async ({ page }) => {
    await expect(page.getByPlaceholder(/image rights agreement/i)).toBeVisible({
        timeout: 10000,
    });
});
...
Then("I should see the attorney wallet address field", async ({ page }) => {
    await expect(page.getByPlaceholder("0x...").nth(1)).toBeVisible({
        timeout: 10000,
    });
});
...
When(
    "I fill the player wallet address with {string}",
...
When(
    "I fill the contract title with {string}",
    async ({ page }, value: string) => {
...
Then(
    "I should see the submit button labeled {string}",
...
Then(
    "the upload zone should mention {string}",
