...
3. Show the user the details and instruct them to click the "Review & Submit Proposal" button/link that is returned. Do NOT attempt to create the agreement directly in the database. All contract creation must go through the user-review flow.`,
            ...
            tools: {
                queryContracts: tool({
                    description:
                        "Queries agreements/contracts related to the current user, optionally filtered by status.",
                    ...
                    execute: async ({ status, limit }) => {
                        ...
                        if (
                            !userDoc ||
                            !userDoc.contracts ||
                            userDoc.contracts.length === 0
                        ) {
                            return { contracts: [] };
                        }
                        const query: any = { _id: { $in: userDoc.contracts } };
                        if (status) query.status = status;
                        const contracts = await Agreement.find(query)
                            .sort({ updatedAt: -1 })
                            .limit(limit ?? 5)
                            .lean();
                        return {
                ...
                queryTransactions: tool({
                    description:
                        "Queries transaction history for the user, optionally filtered by actionType or status.",
                    inputSchema: z.object({
                        actionType: z
                            .string()
                            .optional()
                            .describe(
                                "Filter by action type (e.g. execute_mint, create_vault, etc.)",
                            ),
                        status: z
                            .string()
                            .optional()
                ...
                getAccountInfo: tool({
                    description:
                        "Fetches profile, wallet address, and account details for the currently logged in user.",
                    inputSchema: z.object({}),
                    execute: async () => {
                        await dbConnect();
                ...
                getContractById: tool({
                    description:
                        "Fetches detailed information for a specific agreement/contract by its Mongoose ID.",
                    inputSchema: z.object({
                        id: z
                            .string()
                            .describe(
                                "The unique MongoDB identifier of the contract",
                            ),
                    }),
                    execute: async ({ id }) => {
                        await dbConnect();
                        const userDoc = await User.findById(session.user.id)
                            .select("contracts")
                            .lean();
                        const hasAccess = userDoc?.contracts?.some(
                            (cid) => cid.toString() === id,
                        );
                        if (!hasAccess) {
                            return {
                                error: "Unauthorized or contract not found",
                            };
                        }
                ...
                prepareContract: tool({
                    description:
                        "Prepares a draft contract proposal and returns a structured preview and redirect URL for user confirmation.",
                    inputSchema: z.object({
                        title: z.string().describe("Title of the contract"),
                        description: z
                            .string()
                            .describe(
                                "Detailed description of the image rights agreement",
                            ),
                        playerWalletAddress: z
                            .string()
                            .describe("Ethereum wallet address of the player"),
                        playerEmail: z
                            .string()
                            .describe("Email address of the player"),
                        attorneyWalletAddress: z
                            .string()
                            .describe(
                                "Ethereum wallet address of the attorney",
                            ),
                        attorneyEmail: z
                            .string()
                            .describe("Email address of the attorney"),
                        tokenURI: z
                            .string()
                            .describe(
                                "URI of the metadata or document associated with the contract",
                            ),
                        cautionAmountUSDC: z
                            .string()
                            .describe("Caution deposit amount in USDC"),
                        tokenName: z
                            .string()
                            .describe(
                                "Name of the fractionalized rights token (e.g. PlayerRights)",
                            ),
                        tokenSymbol: z
                            .string()
                            .describe(
                                "Symbol of the fractionalized rights token starting with $ (e.g. $TOKEN_E)",
                            ),
                    }),
                    execute: async (args) => {
                        const prefillData = {
                        ...
                        const base64Prefill = Buffer.from(
                        ...
                        return {
                            ...
                            redirectUrl,
                            message:
                                "Contract draft prepared. Please review and submit using the provided link.",
                        };
                    },
                }),
            },
        ...
        return result.toUIMessageStreamResponse({
            onError: (error) => {
                ...
                if (
                    /model.*(not found|decommission|deprecat)|does not exist/i.test(
                        msg,
                    )
                ) {
