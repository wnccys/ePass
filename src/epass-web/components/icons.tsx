import type { SVGProps } from "react";

export function LinkedinIcon(props: SVGProps<SVGSVGElement>) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            width="24"
            height="24"
            fill="currentColor"
            {...props}
        >
            <path
                fillRule="evenodd"
                d="M5 1.25a2.75 2.75 0 1 0 0 5.5a2.75 2.75 0 0 0 0-5.5M3.75 4a1.25 1.25 0 1 1 2.5 0a1.25 1.25 0 0 1-2.5 0m-1.5 4A.75.75 0 0 1 3 7.25h4a.75.75 0 0 1 .75.75v13a.75.75 0 0 1-.75.75H3a.75.75 0 0 1-.75-.75zm1.5.75v11.5h2.5V8.75zM9.25 8a.75.75 0 0 1 .75-.75h4a.75.75 0 0 1 .75.75v.434l.435-.187a7.8 7.8 0 0 1 2.358-.595C20.318 7.4 22.75 9.58 22.75 12.38V21a.75.75 0 0 1-.75.75h-4a.75.75 0 0 1-.75-.75v-7a1.25 1.25 0 0 0-2.5 0v7a.75.75 0 0 1-.75.75h-4a.75.75 0 0 1-.75-.75zm1.5.75v11.5h2.5V14a2.75 2.75 0 1 1 5.5 0v6.25h2.5v-7.87c0-1.904-1.661-3.408-3.57-3.234a6.3 6.3 0 0 0-1.904.48l-1.48.635a.75.75 0 0 1-1.046-.69V8.75z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function TwitterIcon(props: SVGProps<SVGSVGElement>) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            width="24"
            height="24"
            fill="currentColor"
            {...props}
        >
            <path
                fillRule="evenodd"
                d="M13.796 9.827 21.143 1H19.31L12.784 8.653 7.525 1H1.5L9.255 12.285 1.5 23H3.334l6.837-8.947L15.775 23h6.025L13.796 9.827Zm-2.428 2.775-.817-1.168-5.753-8.232h2.646l4.646 6.647.817 1.168 6.036 8.636h-2.646L11.368 12.602Z"
                clipRule="evenodd"
            />
        </svg>
    );
}

export function MailIcon(props: SVGProps<SVGSVGElement>) {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            width="24"
            height="24"
            fill="currentColor"
            {...props}
        >
            <path
                fillRule="evenodd"
                d="M3 4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h18a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2H3zm0 1.5h18a.5.5 0 0 1 .5.5v.757l-9 5.85-9-5.85V6c0-.276.224-.5.5-.5zM21.5 9.176V18a.5.5 0 0 1-.5.5H3a.5.5 0 0 1-.5-.5V9.176L12 15.35l9.5-6.175z"
                clipRule="evenodd"
            />
        </svg>
    );
}
